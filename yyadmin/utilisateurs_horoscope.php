...
?>
			...
			<span>&raquo;</span>
			<a href="http://www.myastro.fr/yyadmin/utilisateurs_horoscope.php">Utilisateurs Horoscope</a>
			</div>
	...
	<div id="title"><h1>Utilisateurs Horoscope</h1></div>	
</div>
	...
	$db_link = createConnection();
		...
		if(isset($_GET['tri']) && $_GET['tri'] == 'horoscope')
		{
         ...
         $count_where = "AND horoscope = 1";
			$link = $baseSiteAdmin.'utilisateurs_horoscope.php';
		}
		else
		{
		...
		$sql = "SELECT *
FROM `ag_users`
WHERE `horoscope` =1 AND `ip_adress` != 'mobile'
ORDER BY `ag_users`.`internal_id`";
		...
		for($i=1; $i<=$nb_page; $i++)
		{
			$indexation2 .= '<a href="http://www.myastro.fr/yyadmin/utilisateurs_horoscope.php?page='.$i.'">'.$i.'</a> ';
		}
		...
		{
			$indexation .= '
			<a href="http://www.myastro.fr/yyadmin/utilisateurs_horoscope.php?page=1">D&eacute;but</a>
			<a href="http://www.myastro.fr/yyadmin/utilisateurs_horoscope.php?page='.$page_moins.'">Pr&eacute;c&eacute;dent</a>';
		}
		...
		{ 
			$indexation .= '			
			<a href="http://www.myastro.fr/yyadmin/utilisateurs_horoscope.php?page='.$page_plus.'">Suivant</a>
			<a href="http://www.myastro.fr/yyadmin/utilisateurs_horoscope.php?page='.$nb_page.'">Fin</a>';
		}			
...
FROM ag_users
WHERE ag_users.`history` > TIMESTAMP( DATE_ADD(CURDATE( ), INTERVAL -2 DAY) ) AND ag_users.`history` < TIMESTAMP( DATE_ADD(CURDATE( ), INTERVAL -1 DAY) ) AND ag_users.`horoscope` =1 AND `ip_adress`!= 'mobile'") or die ( mysql_error());
		echo 'Vous avez <strong>'.mysql_num_rows($sqlUserJour).' nouveaux utilisateurs</strong> qui se sont inscrits avant-hier.<br><br>';
		$sqlUserJour = mysql_query("SELECT *
FROM ag_users
WHERE ag_users.`history` > TIMESTAMP( DATE_ADD(CURDATE( ), INTERVAL -1 DAY) ) AND ag_users.`history` < TIMESTAMP( CURDATE( ) ) AND ag_users.`horoscope` =1 AND `ip_adress`!= 'mobile'") or die ( mysql_error());
...
AND ag_users.`horoscope` =1 AND `ip_adress`!= 'mobile'") or die ( mysql_error());
		echo 'Vous avez <strong>'.mysql_num_rows($sqlUserJour).' nouveaux utilisateurs</strong> qui se sont inscrits aujourd\'hui.';
		...
		function MoisEnToutesLettres($mois)
		{
			...
			$sqlUserJour = mysql_query("SELECT * FROM ag_users WHERE `horoscope` =1
AND `ip_adress` != 'mobile' AND ag_users.questionDate >= '".$datedebutmois."' ".$count_where."") or die ( mysql_error());
			...
			$datedebutmois=$annee.'-'.$moisencours.'-01';
			$sqlUserJour = mysql_query("SELECT * FROM ag_users WHERE `horoscope` =1
AND `ip_adress` != 'mobile' AND ag_users.questionDate >= '".$datedebutmois."' AND   ag_users.questionDate < '".$datefinmois."'  ".$count_where."") or die ( mysql_error());
...
AND `ip_adress` != 'mobile' AND ag_users.questionDate >= '".$datedebutmois."' AND   ag_users.questionDate < '".$datefinmois."' ".$count_where."") or die ( mysql_error());
       ...
       <div class="indexation" style="text-align:justify; margin: 25px 0;line-height:30px">
		...
		$i += 1;
?>
			<?php if ($row["questionDate"] == $auj) {echo '<tr style="background-color: #FCFFCD">';} else {echo '<tr>';}?>
