...
function read_numerologie($num,$type=0 /*0=>Année, 1=>amour, 2=>personnalité*/, $sexe='f'){

	$sexes=array('h'=>'homme', 'f'=>'femme');
	$text=array(0=>'numerologie_2016_num', 1=>'test_amoureux_numerologie_sexe', 2=>'personnalite_num',3=>'etapes_vie_numero');
...
function read_horoscope_rss($fichier,$mode=0){
	...
	foreach($arr1 as $k=>$v)
	{
		$val=implode('_',explode(' ',str_replace('é','e',mb_strtolower($v[0],'utf8'))));
		if($mode==0)
		{
			$val=explode('_-_',$val);
			$val=$val[1];
...
function read_horoscope_rss2($fichier){
	...
	foreach($arr as $k=>$v)
	{
		...
		$arr2[implode('_',explode(' ',str_replace('é','e',mb_strtolower($arr1[0],'utf8'))))]=str_replace(htmlspecialchars('<em>'),'<br/><br/><em>',str_replace(htmlspecialchars('<br>'),'',htmlspecialchars_decode($arr1[1])));
	}
	$result['desc']=$arr2;
...
function extraction($s_url)
{
	...
	foreach ($lines as $line)
	{
		if (strlen($line) > 0)
		{
			...
			else if (strstr($line, 'Mes conseils pour l\'année'))
			{
				$title = 'conseils';
				$display_next = true;
			}
