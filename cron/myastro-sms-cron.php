...
if ((isset($_GET['testing']) && $_GET['testing'] == 'ok') || (SMS_FLASH_ACTIVATED && $J !== 7))
{	
	
	if ($H === 9 && $J === 1)
	{
		// si $J = lundi (1), à 09h, on envoi les SMS des inscrits entre le samedi 18h et dimanche 17:59:59
		$start_time	 = mktime(18, 0, 0, date('m'), intval(date('d')) - 2, date('Y'));
		$end_time	 = mktime(17, 59, 59, date('m'), intval(date('d')) - 1, date('Y'));
	}
	/*=============  à partir de là , c'est l'envoi habituel ==============================*/
	if ($H === 10)
	{
		// On envoi les SMS des inscrits de 18h00 la veille jusqu'à 8h00
