$(document).ready(function(){
	...
	$('.paquet img').on('click',function(){
		...
		if(!$(this).data('hasBeenClicked') && alreadyPicked < 5){
			...
			if(w >= 480){
				if(w > 979){
					$(this).animate({top:'299',left:Finalposition.left - Relativeposition.left - 138},800);
				}else if(w >= 800 ){
					$(this).animate({top:'299',left:Finalposition.left - Relativeposition.left - 104},800);	
				}else{
					$(this).animate({top:'299',left:Finalposition.left - Relativeposition.left-17},800);
				}
