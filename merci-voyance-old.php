<?php

//include('include/Lib_handler.php');
$page_title = 'Votre demande de voyance a bien été enregistrée.';
include('include/header.php');
?>
<div class="merci">
	<div id="header">
		<p>POUR TOUTE QUESTION URGENTE PROFITEZ D'UNE OFFRE DE BIENVENUE ! </p>
		<p>Consultez un voyant à 10€ les 10 premières minutes : 04 82 53 71 59 </p>
	</div>
	<div id="grishaut">
		<p>CODE PROMO : ASTRO 10</p>
	</div>
	<div id="text">
		<p>Votre question de voyance gratuite a bien été envoyée.</p>
		<p>Vous recevrez votre réponse par e-mail sous 24h.</br>
		Si votre question est urgente, vous pouvez consulter nos voyants au 04 82 53 71 59</br>
		en précisant votre prénom et le code promo : ASTRO10</p>
		<p>Afin de bien recevoir votre étude, insérez notre adresse e-mail dans votre carnet d'adresse.</p>
	<div id="voyant">
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/voyance-par-telephone-valerie">
			<img src="images/merci/valerie.png" alt="Valérie" />
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/voyance-par-telephone-caroline">
			<img src="images/merci/caroline.png" alt="caroline">
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/voyance-par-telephone-cecile">
			<img src="images/merci/cecile.png" alt="cecile">
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/voyance-par-telephone-laurent">
			<img src="images/merci/laurent.png" alt="laurent">
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/voyance-par-telephone-stephane">
			<img src="images/merci/stephane.png" alt="stephane">
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/voyance-par-telephone-georges">
			<img src="images/merci/george.png" alt="george">
		</a>
	</div>
	<img src="images/merci/grisbas.png" />
	<div id="encart">
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/horoscope-gratuit">
			<img src="images/merci/horsgratuit.png" alt="Horocope Gratuit"/>
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/astrologie-gratuite" >
			<img src="images/merci/portrait.jpg" alt="Astrologie Gratuite"/>
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/tarot-gratuit">
			<img src="images/merci/tarot.jpg" alt="Tarot Gratuit"/>
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/voyance-par-telephone">
			<img src="images/merci/tel.jpg" alt="Voyance par Téléphone"/>
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/numerologie-gratuite">
			<img src="images/merci/sms.png" alt="Numérologie Gratuite"/>
		</a>
		<a href="<?= PROTOCOL.'://'.ROOT_URL ?>/forum/index">
			<img src="images/merci/cb.jpg" alt="forum"/>
		</a>
	</div>
</div>
<!-- Google Code for MY ASTRO QUESTION INSCRIPTION Conversion Page -->
<script type="text/javascript">
/* <![CDATA[ */
var google_conversion_id = 1024378253;
var google_conversion_language = "fr";
var google_conversion_format = "1";
var google_conversion_color = "ffffff";
var google_conversion_label = "arTgCM2fhAMQjYu76AM";
var google_conversion_value = 1.00;
var google_conversion_currency = "EUR";
var google_remarketing_only = false;
/* ]]> */
</script>
<script type="text/javascript" src="//www.googleadservices.com/pagead/conversion.js">
</script>
<noscript>
<div style="display:inline;">
<img height="1" width="1" style="border-style:none;" alt="" src="//www.googleadservices.com/pagead/conversion/1024378253/?value=1.00&amp;currency_code=EUR&amp;label=arTgCM2fhAMQjYu76AM&amp;guid=ON&amp;script=0"/>
</div>
</noscript>

<!-- Facebook Conversion Code for Leads -->
<script>(function() {
  var _fbq = window._fbq || (window._fbq = []);
  if (!_fbq.loaded) {
    var fbds = document.createElement('script');
    fbds.async = true;
    fbds.src = '//connect.facebook.net/en_US/fbds.js';
    var s = document.getElementsByTagName('script')[0];
    s.parentNode.insertBefore(fbds, s);
    _fbq.loaded = true;
  }
})();
window._fbq = window._fbq || [];
window._fbq.push(['track', '6017569293945', {'value':'0.01','currency':'EUR'}]);
</script>
<noscript><img height="1" width="1" alt="" style="display:none" src="https://www.facebook.com/tr?ev=6017569293945&amp;cd[value]=0.01&amp;cd[currency]=EUR&amp;noscript=1" /></noscript>


<!-- Analytics tags -->
    <script type="text/javascript">

        var _gaq = _gaq || [];
        _gaq.push(['_setAccount', 'UA-16975536-3']);
        _gaq.push(['_trackPageview']);

        (function () {
            var ga = document.createElement('script');
            ga.type = 'text/javascript';
            ga.async = true;
            ga.src = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'stats.g.doubleclick.net/dc.js';
            var s = document.getElementsByTagName('script')[0];
            s.parentNode.insertBefore(ga, s);
        })();

    </script>  
<?php
	include('include/footer.php');
?>
   